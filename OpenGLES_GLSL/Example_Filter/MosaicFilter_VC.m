...
//  OpenGLES_GLSL
//
//  Created by SPIREJ on 2019/6/27.
...
@interface MosaicFilter_VC ()<FilterBarDelegate>
...
@implementation MosaicFilter_VC
...
- (void)setupFilterBar {
    ...
    NSArray *dataSource = @[@"无",@"灰度",@"浮雕",@"漩涡",@"矩形马赛克",@"六边形马赛克",@"三角形马赛克",@"圆形马赛克"];
...
- (void)filterInit {
    ...
    //6.获取处理的图片路径
//    NSString *imagePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"gril.jpg"];
    //读取图片
...
- (void)filterBar:(FilterBar *)filterBar didScrollToIndex:(NSUInteger)index {
    ...
    if (index == 0) {
        ...
        [self setupReliefShaderProgram];
    }else if (index == 3) {
        [self setupVortexShaderProgram];
    }else if (index == 4) {
        [self setupRectangleShaderProgram];
    }else if (index == 5) {
        [self setupSexangleShaderProgram];
    }else if (index == 6) {
        [self setupTriangleShaderProgram];
    }else if (index == 7) {
        [self setupCircleShaderProgram];
    }
    // 重新开始滤镜动画
    [self startFilerAnimation];
}
...
// 矩形马赛克滤镜着色器程序
- (void)setupRectangleShaderProgram {
    //设置着色器程序
    [self setupShaderProgramWithName:@"Rectangle"];
}
    
// 六边形马赛克滤镜着色器程序
- (void)setupSexangleShaderProgram {
    //设置着色器程序
    [self setupShaderProgramWithName:@"Sexangle"];
}
    
// 三角形马赛克滤镜着色器程序
- (void)setupTriangleShaderProgram {
    //设置着色器程序
    [self setupShaderProgramWithName:@"Triangle"];
}
