...
//  OpenGLES_GLSL
//
//  Created by SPIREJ on 2019/6/25.
...
@interface SplitScreen_VC ()<FilterBarDelegate>
...
- (void)setFilterBar {
    FilterBar *filterBar = [[FilterBar alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-100, self.view.frame.size.width, 100)];
    filterBar.delegate = self;
    filterBar.itemList = @[@"正常", @"分2屏", @"分3屏", @"分4屏", @"分6屏", @"分9屏"];
...
//    self.vertices[0] = (SenceVertex){{-1, 1, 0}, {0, 1}};
//    self.vertices[1] = (SenceVertex){{-1, -1, 0}, {0, 0}};
//    self.vertices[2] = (SenceVertex){{1, -1, 0}, {1, 0}};
//    self.vertices[3] = (SenceVertex){{1, 1, 0}, {1, 1}};
    ...
    //9.设置默认着色器
    [self setupNormalShaderProgram]; //开始选用默认的着色器程序
...
- (void)bindRenderLayer:(CALayer <EAGLDrawable> *)layer {
    ...
    //2.获取渲染缓存区名称，绑定渲染缓存区l以及将渲染缓存区与layer建立连接
    glGenRenderbuffers(1, &renderBuffer);
    glBindRenderbuffer(GL_RENDERBUFFER, renderBuffer);
    [self.context renderbufferStorage:GL_RENDERBUFFER fromDrawable:layer];
    
    //3.获取帧缓存区名称，绑定帧缓存以及将渲染缓存区附着到帧缓存区上
...
- (void)filterBar:(FilterBar *)filterBar didScrollToIndex:(NSUInteger)index {
    switch (index) {
        case 0:
            ...
            break;
        case 1:
            [self setupSplitScreen_2_ShaderProgram];
            break;
        case 2:
            [self setupSplitScreen_3_ShaderProgram];
            break;
        case 3:
            [self setupSplitScreen_4_ShaderProgram];
            break;
        case 4:
            [self setupSplitScreen_6_ShaderProgram];
            break;
        case 5:
            [self setupSplitScreen_9_ShaderProgram];
            break;
...
- (void)setupSplitScreen_2_ShaderProgram {
    [self setupShaderProgramWithName:@"SplitScreen_2"];
}

- (void)setupSplitScreen_3_ShaderProgram {
    [self setupShaderProgramWithName:@"SplitScreen_3"];
}

- (void)setupSplitScreen_4_ShaderProgram {
    [self setupShaderProgramWithName:@"SplitScreen_4"];
}

- (void)setupSplitScreen_6_ShaderProgram {
    [self setupShaderProgramWithName:@"SplitScreen_6"];
}

- (void)setupSplitScreen_9_ShaderProgram {
